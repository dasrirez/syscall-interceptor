...
MODULE_AUTHOR("Darius Asri Rezaei, Jason Zhong, Omid Najmeddini");
MODULE_LICENSE("GPL");
...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    /* according to a1 doc */
    /* when monitoring is MONITORED the pidlist is a white list */
    /* when monitoring is ALL_MONITORED the pidlist is a black list */
    pid_in_list = check_pid_monitored(reg.ax, current->pid);
    /* printk("table[reg.ax].monitored == %d && check_pid_monitored(reg.ax, %d) == %d \n", table[reg.ax].monitored, (int)current->pid, pid_in_list); */
    if ((table[reg.ax].monitored == MONITORED && pid_in_list == 1)
        || (table[reg.ax].monitored == ALL_MONITORED && pid_in_list == 0)) {
...
int validate_my_syscall(int cmd, int syscall, int pid) {
    spin_lock(&calltable_lock);
    spin_lock(&pidlist_lock);
    /* printk("my_syscall %d %d %d\n", cmd, syscall, pid); */
    if (!(pid >= 0 && syscall > 0 && syscall < NR_syscalls && syscall != MY_CUSTOM_SYSCALL)) {
        spin_unlock(&calltable_lock);
        spin_unlock(&pidlist_lock);
        return -EINVAL;
    }
    ...
    if (cmd == REQUEST_START_MONITORING) {
        ...
        /* pid should not be in white list */
    ...
    if (cmd == REQUEST_STOP_MONITORING) {
        /* pid should not be in black list */
        if (pid > 0 && table[syscall].monitored == ALL_MONITORED) {
            if (check_pid_monitored(syscall, pid == 1)) {
                spin_unlock(&calltable_lock);
                spin_unlock(&pidlist_lock);
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    int validation_code = validate_my_syscall(cmd, syscall, pid);
    ...
    if (cmd == REQUEST_SYSCALL_INTERCEPT) {
        ...
        if (pid == 0) {
            ...
            int exit_code;
            /* pidlist is black list */
            if (table[syscall].monitored == ALL_MONITORED) {
                exit_code = del_pid_sysc(pid, syscall);
            } 
            /* pidlist is white list */
            else {
                exit_code = add_pid_sysc(pid, syscall);
...
static int init_function(void) {
    ...
    for (s = 1; s < NR_syscalls + 1; s++) {
        table[s].f = sys_call_table[s];
        table[s].intercepted = NOT_INTERCEPTED;
        table[s].monitored = NOT_MONITORED;
        table[s].listcount = 0;
...
static void exit_function(void)
{
    ...
    set_addr_ro((unsigned long) sys_call_table);
    /* printk("orig_exit_group [%lx] :: table[__NR_exit_group].f [%lx]\n", orig_exit_group, table[__NR_exit_group].f); */

    for (s = 1; s < NR_syscalls + 1; s++) {
        if (s != MY_CUSTOM_SYSCALL && s != __NR_exit_group) {
            /* printk("syscall [%lx] :: table[__NR_exit_group].f [%lx] \n", sys_call_table[s], table[s].f); */
            set_addr_rw((unsigned long) sys_call_table);
            sys_call_table[s] = table[s].f;
            set_addr_ro((unsigned long) sys_call_table);
            destroy_list(s);
